...
func UserAsTeamIncludedItems(v *User) TeamIncludedItems {
	return TeamIncludedItems{User: v}
}
...
func (dst *TeamIncludedItems) UnmarshalJSON(data []byte) error {
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(TeamIncludedItems)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(TeamIncludedItems)")
	}
}
...
func NewNullableTeamIncludedItems(val *TeamIncludedItems) *NullableTeamIncludedItems {
	return &NullableTeamIncludedItems{value: val, isSet: true}
}
