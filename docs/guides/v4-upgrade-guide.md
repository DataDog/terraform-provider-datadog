...
**Note:** If your organization has One-Time Read Only mode enabled for Application Keys, then no action is needed to migrate for this resource because import is already unavailable.

**Before (v3.x):**

```shell
# Previously allowed (with deprecation warning)
