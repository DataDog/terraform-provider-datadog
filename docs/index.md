...
- `default_tags` (Block List, Max: 1) [Experimental - Cloud Configuration Rule, Logs Pipelines, Monitors Security Monitoring Rule, Sensitive Data Scanner Rule, and Service Level Objective only] Configuration block containing settings to apply default resource tags across all resources. (see [below for nested schema](#nestedblock--default_tags))
...
- `tags` (Map of String) [Experimental - Cloud Configuration Rule, Logs Pipelines, Monitors Security Monitoring Rule, Sensitive Data Scanner Rule, and Service Level Objective only] Resource tags to be applied by default across all resources.
