...
- `default_tags` (Block List, Max: 1) Configuration block containing settings to apply default resource tags across all compatible resources (monitor, security_monitoring_rule, sensitive_data_scanner_rule, service_level_objective, synthetics_test, synthetics_global_variable). (see [below for nested schema](#nestedblock--default_tags))
...
- `tags` (Map of String) Resource tags across all compatible resources (monitor, security_monitoring_rule, sensitive_data_scanner_rule, service_level_objective, synthetics_test, synthetics_global_variable).
