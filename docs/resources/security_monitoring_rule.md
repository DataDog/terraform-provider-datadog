...
- `has_optional_group_by_fields` (Boolean) When false, the rule ignores events without a group-by value. When true, events with missing group-by fields are processed with `N/A`, replacing the missing values. Defaults to `false`.
