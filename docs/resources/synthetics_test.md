...
- `monitor_priority` (Number)
- `no_screenshot` (Boolean) Prevents saving screenshots of the steps.
- `restricted_roles` (Set of String, Deprecated) (This field is no longer supported by Datadog API. Please use `datadog_restriction_policy instead) A list of role identifiers pulled from the Roles API to restrict read and write access. **Deprecated.** This field is no longer supported by the Datadog API. Please use `datadog_restriction_policy instead.
