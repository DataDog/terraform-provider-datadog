...
- **assertions** (List of Map of String, Optional, Deprecated) **DEPRECATED** Define `assertion` blocks instead. List of assertions.
...
- **step** (Block List, Deprecated) **DEPRECATED** Define `browser_step` blocks instead. Steps for browser tests. (see [below for nested schema](#nestedblock--step))
...
- **variable** (Block List, Deprecated) **DEPRECATED** Define `browser_variable` blocks instead. Variables used for a browser test steps. Multiple `browser_variable` blocks are allowed with the structure below. (see [below for nested schema](#nestedblock--variable))
