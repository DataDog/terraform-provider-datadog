...
- `destination_bucket_region` (String) AWS Region of the bucket holding the inventory files. Required when cloud_provider is `aws`.
...
- `gcp_source_bucket_name` (String) Name of the GCP bucket the inventory report is generated for. Required when cloud_provider is `gcp`.
