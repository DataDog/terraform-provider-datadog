...
`policy_name` can be changed in place for both `org_config` and `role` policies without replacing the resource.
...
- `policy_name` (String) The name of the policy. This becomes the name of the resource created across orgs in the group (for example, for `role` policies, the name of the created role). String length must be at least 1.
