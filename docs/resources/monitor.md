...
- `locked` (Boolean, Deprecated) A boolean indicating whether changes to this monitor should be restricted to the creator or admins. Defaults to `false`. **Deprecated.** Use `datadog_restriction_policy` resource to manage monitor permissions instead.
