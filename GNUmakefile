...
check-docs: docs
	@if [ "`git status --porcelain docs/`" ] ; then echo "Uncommitted changes were detected in the autogenerated docs folder. Please run 'make docs' to autogenerate the docs" && echo `git status --porcelain docs/` && exit 1; else echo "Success: No generated documentation changes detected"; fi
