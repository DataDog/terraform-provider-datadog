...
const datadogDashboardFreeTextConfigDeprecated = `
resource "datadog_dashboard" "free_text_dashboard" {
	title         = "{{uniq}}"
	description   = "Created using the Datadog provider in Terraform"
	layout_type   = "free"
...
var datadogDashboardFreeTextAssertsDeprecated = []string{
	"widget.0.layout.y = 5",
	"widget.0.free_text_definition.0.text = Free Text",
	"layout_type = free",
	"description = Created using the Datadog provider in Terraform",
...
func TestAccDatadogDashboardFreeTextDeprecated(t *testing.T) {
	testAccDatadogDashboardWidgetUtil(t, datadogDashboardFreeTextConfigDeprecated, "datadog_dashboard.free_text_dashboard", datadogDashboardFreeTextAssertsDeprecated)
}
