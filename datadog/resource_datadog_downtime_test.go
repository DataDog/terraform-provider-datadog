			...
			for _, r := range s.RootModule().Resources {
				if r.Primary.ID != "" {
					if dt, httpResp, err := datadogClientV1.DowntimesApi.GetDowntime(authV1, int64(id)).Execute(); err != nil {
						...
						return &FatalError{prob: fmt.Sprintf("received an error retrieving Downtime %s", err)}
					} else {
					...
					return &RetryableError{prob: fmt.Sprintf("Downtime still exists or is active")}
				}
			}
			return nil
		})
