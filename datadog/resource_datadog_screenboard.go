	...
	tileDefRequest := &schema.Schema{
		...
		Elem: &schema.Resource{
			Schema: map[string]*schema.Schema{
				...
				"style": {
					Description: "Only for widgets of type \"timeseries\", \"query_value\", \"toplist\", \"process\": describing how to display the widget. The structure of this block is described below. At most one such block should be present in a given request block.",
					Type:        schema.TypeMap,
					Optional:    true,
				},
				...
				"metadata_json": {
					Description:  "A JSON blob (preferrably created using jsonencode) representing mapping of query expressions to alias names. Note that the query expressions in metadata_json will be ignored if they're not present in the query.",
					Type:         schema.TypeString,
					Optional:     true,
