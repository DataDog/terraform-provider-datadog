...
// destinations (amazon_s3, google_cloud_storage, azure_storage). It is a tagged union keyed on
// `algorithm` with variants gzip/zstd.
	...
	return schema.ListNestedBlock{
		...
		NestedObject: schema.NestedBlockObject{
			Attributes: map[string]schema.Attribute{
				"algorithm": schema.StringAttribute{
					Required:    true,
					Description: "Compression algorithm. One of `gzip` or `zstd`.",
