...
var (
	_ datasource.DataSourceWithConfigure = &datadogDatastoreDataSource{}
)
...
func (d *datadogDatastoreDataSource) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	...
	if !state.ID.IsNull() {
		resp, _, err := d.Api.GetDatastore(d.Auth, state.ID.ValueString())
		if err != nil {
			response.Diagnostics.Append(utils.FrameworkErrorDiag(err, "error reading datastore"))
			return
		}
