...
func flattenProcessorGroup(ctx context.Context, group *datadogV2.ObservabilityPipelineConfigProcessorGroup) *processorGroupModel {
	...
	for _, p := range processors {
		...
		if p.ObservabilityPipelineFilterProcessor != nil {
			...
			id = p.ObservabilityPipelineMetricTagsProcessor.GetId()
			enabled = p.ObservabilityPipelineMetricTagsProcessor.GetEnabled()
			include = p.ObservabilityPipelineMetricTagsProcessor.GetInclude()
		}
		...
		if f := flattenMetricTagsProcessorItem(ctx, p.ObservabilityPipelineMetricTagsProcessor); f != nil {
			procModel.MetricTagsProcessor = append(procModel.MetricTagsProcessor, f)
		}
	}
...
func expandProcessorTypes(ctx context.Context, processor *processorModel) []datadogV2.ObservabilityPipelineConfigProcessorItem {
	...
	for _, p := range processor.MetricTagsProcessor {
		items = append(items, expandMetricTagsProcessorItem(ctx, procId, procEnabled, procInclude, p))
	}

	return items
}

func expandMetricTagsProcessorItem(ctx context.Context, id string, enabled bool, include string, src *metricTagsProcessorModel) datadogV2.ObservabilityPipelineConfigProcessorItem {
...
func flattenMetricTagsProcessorItem(ctx context.Context, src *datadogV2.ObservabilityPipelineMetricTagsProcessor) *metricTagsProcessorModel {
	if src == nil {
		return nil
	}

	out := &metricTagsProcessorModel{}
	for _, rule := range src.GetRules() {
		var keys []types.String
		for _, k := range rule.GetKeys() {
			keys = append(keys, types.StringValue(k))
		}
		out.Rules = append(out.Rules, metricTagsProcessorRuleModel{
