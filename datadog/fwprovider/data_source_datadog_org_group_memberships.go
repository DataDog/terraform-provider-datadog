...
func (d *datadogOrgGroupMembershipsDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, response *datasource.SchemaResponse) {
	response.Schema = schema.Schema{
		Description: "Use this data source to retrieve org group memberships. At least one filter (`org_group_id` or `org_uuid`) is required. **Note**: Org Groups operations are in preview and subject to change. Access may need to be enabled for your organization. Contact [Datadog support](https://docs.datadoghq.com/help/) to request access.",
		Attributes: map[string]schema.Attribute{
			"id": utils.ResourceIDAttribute(),
			"org_group_id": schema.StringAttribute{
				Optional:    true,
				Description: "Filter memberships to those within the given org group. At least one filter (`org_group_id` or `org_uuid`) is required.",
				Validators:  []validator.String{uuidValidator},
			},
			"org_uuid": schema.StringAttribute{
				Optional:    true,
				Description: "Filter memberships to those for the given organization. At least one filter (`org_group_id` or `org_uuid`) is required.",
				Validators:  []validator.String{uuidValidator},
			},
