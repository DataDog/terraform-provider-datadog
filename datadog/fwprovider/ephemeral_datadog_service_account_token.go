...
type serviceAccountTokenEphemeralModel struct {
	...
	RotationTrigger  types.Int64  `tfsdk:"rotation_trigger"`
...
func (r *serviceAccountTokenEphemeralResource) Schema(_ context.Context, _ ephemeral.SchemaRequest, resp *ephemeral.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"rotation_trigger": schema.Int64Attribute{
				Required:    true,
				Description: "Rotation control suffix for the SAT name. Bumping this value (e.g., 1 → 2, or wiring from `time_rotating.<name>.unix`) produces a new SAT on the next apply. Pair this with the lifecycle resource's `key_id_version` so plan-diffing fires Update on rotation.",
			},
...
func (r *serviceAccountTokenEphemeralResource) Open(ctx context.Context, req ephemeral.OpenRequest, resp *ephemeral.OpenResponse) {
	...
	actualName := constructSATName(cfg.Prefix.ValueString(), cfg.Anchor.ValueString(), cfg.RotationTrigger.ValueInt64())
...
func constructSATName(prefix, anchor string, rotationTrigger int64) string {
	return fmt.Sprintf("%s-%s-%d", prefix, anchor, rotationTrigger)
}
