...
func TestAccIntegrationAwsAccountBasic(t *testing.T) {
	t.Parallel()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{ // create account
				Config: testAccCheckDatadogIntegrationAwsAccount(accountID, uniq),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"datadog_integration_aws_account.foo", "auth_config.aws_auth_config_role.role_name", "test"),
					resource.TestCheckResourceAttr(
					...
					resource.TestCheckResourceAttr(
						"datadog_integration_aws_account.foo", "resources_config.extended_collection", "false"),
					resource.TestCheckResourceAttr(
						"datadog_integration_aws_account.foo", "traces_config.xray_services.include_all", "true"),
				),
			}, { // update account
				Config: testAccCheckDatadogIntegrationAwsAccountUpdateConfig(accountID, uniq),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDatadogIntegrationAwsAccountExists(providers.frameworkProvider),
					resource.TestCheckResourceAttr(
						"datadog_integration_aws_account.foo", "aws_account_id", accountID),
					...
					resource.TestCheckResourceAttr(
						"datadog_integration_aws_account.foo", "metrics_config.namespace_filters.exclude_only.#", "0"),
					resource.TestCheckResourceAttr(
...
func testAccCheckDatadogIntegrationAwsAccount(accountID, uniq string) string {
	return fmt.Sprintf(`
...
func testAccCheckDatadogIntegrationAwsAccountUpdateConfig(accountID, uniq string) string {
	return fmt.Sprintf(`
resource "datadog_integration_aws_account" "foo" {
    aws_account_id = %s
    account_tags = []
