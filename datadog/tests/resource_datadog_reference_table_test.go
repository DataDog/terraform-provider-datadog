...
// TestAccReferenceTable_UnknownSchemaFields tests that reference tables can be created
// when schema.fields values come from unknown sources (computed attributes from other resources).
// This test reproduces the "Received unknown value, however the target type cannot handle unknown values"
// error that occurs when the Fields slice is a Go slice instead of types.List.
func TestAccReferenceTable_UnknownSchemaFields(t *testing.T) {
	t.Parallel()
	ctx, providers, accProviders := testAccFrameworkMuxProviders(context.Background(), t)
	uniq := uniqueEntityName(ctx, t)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				// This config creates two reference tables where the second one's schema
				// fields depend on the first one's ID (an unknown value at plan time).
				// This should trigger the unknown value handling path.
				Config: testAccCheckDatadogReferenceTableWithUnknownValues(uniq),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDatadogReferenceTableExists(providers.frameworkProvider),
					resource.TestCheckResourceAttr(
						"datadog_reference_table.primary", "schema.fields.0.name", "a"),
					resource.TestCheckResourceAttr(
						"datadog_reference_table.secondary", "schema.fields.0.name", "x"),
				),
			},
		},
	})
...
func testAccCheckDatadogReferenceTableWithUnknownValues(uniq string) string {
	sanitized := strings.ToLower(strings.ReplaceAll(uniq, "-", "_"))
	return fmt.Sprintf(`
# First table - creates an ID that will be used as an unknown value
resource "datadog_reference_table" "primary" {
  table_name  = "tf_test_primary_%s"
  description = "Primary table for unknown value test"
  source      = "S3"
  ...
  tags = ["test:unknown-values"]
}

# Second table - uses an expression that depends on the first table's ID
# This forces the schema block to be evaluated with unknown values during planning
resource "datadog_reference_table" "secondary" {
  table_name  = "tf_test_secondary_%s"
  # Description uses an unknown value (the primary table's ID) which forces
  # Terraform to mark other computed attributes as unknown during planning
  description = "Secondary table linked to ${datadog_reference_table.primary.id}"
  source      = "S3"
