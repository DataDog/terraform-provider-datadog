...
resource "datadog_user" "user_for_team_rules" {
  email = "USER_EMAIL"
}

resource "datadog_team" "team_for_team_rules" {
  description = "Description"
  handle      = "TEAM_HANDLE"
  name        = "TEAM_NAME"
}

resource "datadog_on_call_schedule" "schedule_for_team_rules" {
...
resource "datadog_on_call_escalation_policy" "policy_for_team_rules" {
  name                        = "POLICY_NAME"
