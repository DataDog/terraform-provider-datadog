...
func TestAccDatadogAppResource_FromFile_Complex(t *testing.T) {
	...
	appJSON, err := json.Marshal(testComplexAppJSON(appName))
	if err != nil {
...
func testComplexAppJSON(name string) string {
	return fmt.Sprintf(`
	{
		...
		"description": "Create new tickets in Jira. Test app created using the Datadog provider in Terraform",
		"name": "%s",
