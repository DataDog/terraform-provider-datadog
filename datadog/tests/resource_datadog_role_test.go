		...
		for _, r := range s.RootModule().Resources {
			...
			if err != nil {
				if !(httpresp != nil && httpresp.StatusCode == 404) {
					return utils.TranslateClientError(err, httpresp.Request.URL, "error getting role")
				}
...
func testAccCheckDatadogRoleExists(accProvider func() (*schema.Provider, error), rolename string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if err != nil {
			return utils.TranslateClientError(err, httpresp.Request.URL, "error checking role existence")
		}
		return nil
	}
}
