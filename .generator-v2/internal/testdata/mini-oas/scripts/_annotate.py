...
For each sample, two annotated copies may be written from the same slice:
  - gen-test/<name>.yaml   singular — the by-id GET (path has a {param}) as read;
  - gen-test/<plural>.yaml  plural  — the collection GET (no {param}) as read,
                                      carrying cardinality: plural.
A slice contributes only the variants its endpoints support: a by-id-only slice
yields just the singular, a list-only slice (e.g. users) just the plural.
"""
import os
import yaml
...
def write_annotated(name, artifact_name, op, plural, description):
    """Load the pristine slice, annotate op as a data source, write to gen-test/."""
    spec = yaml.load(open(os.path.join(MINI_OAS_DIR, f"mini-datadog_{name}.yaml")), Loader=Loader)
    path, method, op_id = op
    ext = {
    ...
    print(f"annotated {artifact_name:<16} {kind:<8} read={op_id:<28} {method.upper()} {path}")
...
def main():
    ...
    for name in SAMPLE:
        ...
        if read:
            write_annotated(name, name, read, plural=False,
                            description=f"Use this data source to retrieve information about an existing Datadog {thing}.")

        listed = pick_list_op(paths)
        if listed:
            write_annotated(name, pluralize(name), listed, plural=True,
                            description=f"Use this data source to retrieve information about existing Datadog {thing}s.")
