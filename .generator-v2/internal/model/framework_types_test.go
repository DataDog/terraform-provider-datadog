...
var _ = Describe("FrameworkType", func() {
	...
	DescribeTable("maps a representable schema node to its framework type strings",
		...
		Entry("an unconstrained value becomes normalized JSON text",
			&Schema{Kind: SchemaKindJSON},
			"schema.StringAttribute", "jsontypes.Normalized"),
...
var _ = Describe("ElementType", func() {
	...
	DescribeTable("maps a primitive-terminal element/value schema to its framework attr.Type",
		...
		Entry("a JSON element becomes a normalized JSON custom type",
			&Schema{Kind: SchemaKindJSON}, "jsontypes.NormalizedType{}"),
