	...
	return DataSourceView{
		...
		State: StateView{
			...
			ItemFields: []StateAssignment{
				{LHS: "Description", RHS: "types.StringValue(item.GetAttributes().GetDescription())"},
				{LHS: "Handle", RHS: "types.StringValue(item.GetAttributes().GetHandle())"},
				{LHS: "ID", RHS: "types.StringValue(item.GetId())"},
				{LHS: "LinkCount", RHS: "types.Int64Value(int64(item.GetAttributes().GetLinkCount()))"},
				{LHS: "Name", RHS: "types.StringValue(item.GetAttributes().GetName())"},
				{LHS: "Summary", RHS: "types.StringValue(item.GetAttributes().GetSummary())"},
				{LHS: "UserCount", RHS: "types.Int64Value(int64(item.GetAttributes().GetUserCount()))"},
			},
			IDHashExpr: `utils.ConvertToSha256(fmt.Sprintf("%s:%t", state.FilterKeyword.ValueString(), state.FilterMe.ValueBool()))`,
		},
	}
}
