...
# Single source of truth for the rotation knob. Bump this int (1 -> 2 -> 3) to rotate.
# For scheduled rotation, wire it to e.g. time_rotating.monthly.unix instead of a literal.
locals {
...
# key_id_version is the non-ephemeral int paired with the write-only key_id (same
# pair-of-fields pattern as AWS Secrets Manager's secret_string_wo / secret_string_wo_version).
# Bumping rotation_trigger ticks key_id_version → plan diffs → Update fires → fresh key_id read.
resource "datadog_service_account_token_lifecycle" "ci" {
  service_account_id = datadog_service_account.demo.id
  key_id             = ephemeral.datadog_service_account_token.ci.id
  key_id_version     = local.rotation_trigger
  retain_count       = 1
}
...
output "rotation_trigger"    { value = local.rotation_trigger }
