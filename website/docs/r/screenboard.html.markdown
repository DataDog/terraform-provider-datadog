...
- `columns` - (Optional, only for widgets of type "log_stream") Stringified list of columns to use. Example: `"[\"column1\",\"column2\",\"column3\"]"` **Note**: When creating a column on a facet from an attribute prefixed with `@`, change the prefix to `log_`. For example `@http.status_code` would become `log_http.status_code` in your terraform config.
